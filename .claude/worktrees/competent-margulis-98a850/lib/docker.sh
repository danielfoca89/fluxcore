...
[[ -n "${_FLUXCORE_DOCKER_LOADED:-}" ]] && return 0
readonly _FLUXCORE_DOCKER_LOADED=1
...
# Check Docker installation state and auto-start the daemon if it is stopped.
# Returns: 0 = running, 1 = not installed, 2 = installed but won't start.
check_docker() {
...
# Prints "docker compose" (plugin) or "docker-compose" (standalone).
...
# Find an available subnet in Docker's private range (172.16–31.0.0/16).
# Scans existing networks to avoid conflicts, then returns the first free slot.
# Output format: "SUBNET:GATEWAY"  e.g. "172.20.0.0/16:172.20.0.1"
# Returns 1 if all 16 slots are occupied.
find_available_subnet() {
    log_info "Finding available subnet..." >&2
    
    # Get all existing subnets
    local existing_subnets
    existing_subnets=$(run_sudo docker network inspect $(run_sudo docker network ls -q) 2>/dev/null | grep '"Subnet"' | awk -F'"' '{print $4}' | sort -u) || existing_subnets=""
    ...
    log_error "No available subnets in Docker's private range (172.16-31.0.0/16)" >&2
    log_info "Existing subnets:" >&2
    echo "$existing_subnets" | while read -r subnet; do
        [[ -n "$subnet" ]] && log_info "  - $subnet" >&2
    done
    return 1
}
...
create_docker_network() {
    ...
    if ! run_sudo docker network inspect "$network_name" &> /dev/null; then
        ...
        local subnet_info
        subnet_info=$(find_available_subnet) || { log_error "Failed to find available subnet"; return 1; }

        local subnet gateway
        subnet=$(echo "$subnet_info" | cut -d: -f1)
        gateway=$(echo "$subnet_info" | cut -d: -f2)
...
deploy_with_compose() {
    ...
    local compose_cmd
    compose_cmd=$(check_docker_compose) || { log_error "Docker Compose not available"; return 1; }
...
# Poll container health until it reports "healthy" or the timeout expires.
# Falls back to checking State.Running for containers without a HEALTHCHECK.
# Returns: 0 = healthy/running, 1 = unhealthy/missing, 2 = timed out.
# Args: $1 = container_name, $2 = max_attempts (default 30, ~60s at 2s per poll)
check_container_health() {
    ...
    while [[ $attempt -lt $max_attempts ]]; do
        ...
        if [[ -n "$health_status" ]]; then
            ...
            local is_running
            is_running=$(run_sudo docker inspect --format='{{.State.Running}}' "$container_name" 2>/dev/null)
...
# Pull a Docker image with exponential-backoff retry (5s → 10s → 20s).
# Handles Docker Hub rate limits and transient network errors.
# Args: $1 = image_name, $2 = max_retries (default 3)
pull_docker_image() {
    local image_name=$1
    ...
    while [[ $attempt -le $max_retries ]]; do
        ...
        if run_sudo docker pull "$image_name" 2>&1; then
            ...
            if run_sudo docker pull "$image_name" 2>&1 | grep -qi "rate limit\|too many requests"; then
                log_warn "Docker Hub rate limit — waiting extra before retry..."
                sleep $((retry_delay * 2))
            fi
        fi
