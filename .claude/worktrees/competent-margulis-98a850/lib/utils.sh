...
# Common functions for logging, sudo handling, and system operations
# ==============================================================================

# Prevent double-sourcing
[[ -n "${_FLUXCORE_UTILS_LOADED:-}" ]] && return 0
readonly _FLUXCORE_UTILS_LOADED=1

set -Eeuo pipefail

# --- COLORS FOR LOGGING ---
# Use constants.sh values if available, otherwise define defaults
RED="${FC_RED:-\033[0;31m}"
GREEN="${FC_GREEN:-\033[0;32m}"
YELLOW="${FC_YELLOW:-\033[1;33m}"
BLUE="${FC_BLUE:-\033[0;34m}"
CYAN="${FC_CYAN:-\033[0;36m}"
MAGENTA="${FC_MAGENTA:-\033[0;35m}"
NC="${FC_NC:-\033[0m}" # No Color


# --- AUDIT LOGGING ---
AUDIT_LOG="${FLUXCORE_AUDIT_LOG:-${HOME}/.vps-secrets/.audit.log}"
...
audit_log() {
    ...
    if [[ -n "$details" ]]; then
        printf "[%s] %s %s by %s - %s - %s\n" "$timestamp" "$action" "$app_name" "$user" "$details" "$result" >> "$AUDIT_LOG"
    else
        printf "[%s] %s %s by %s - %s\n" "$timestamp" "$action" "$app_name" "$user" "$result" >> "$AUDIT_LOG"
    fi
}
...
# Fallback: define get_ssh_service_name only when os-detect.sh is not loaded.
# The authoritative version lives in os-detect.sh.
...
# Fallback: define service_exists only when os-detect.sh is not loaded.
if ! declare -f service_exists >/dev/null 2>&1; then
    service_exists() {
        local service_name=$1
...
# Priority: 1) already root → exec directly, 2) SUDO_PASS env var (automation),
#           3) docker group shortcut, 4) interactive / passwordless sudo.
run_sudo() {
...
# --- OS DETECTION ---
# Fallback detect_os used only when os-detect.sh is not loaded.
# The authoritative version in os-detect.sh also sets OS_FAMILY, OS_VERSION_ID, etc.
...
cleanup_temp_files() {
    local temp_dir="/tmp/flux-orchestrator-$$"
...
# Harden sudo: 30-min password cache + full I/O command logging.
...
# Admin group: full sudo access, requires password but caches it for 30 minutes
%sudo ALL=(ALL:ALL) ALL
%wheel ALL=(ALL:ALL) ALL

# Set sudo timeout to 30 minutes to balance security and convenience
...
EOF
    log_success "sudo configured: 30-min password cache for admin group + full command logging → /var/log/sudo.log"
}
...
init_logging() {
    local log_dir="${HOME}/.flux-orchestrator/logs"
...
setup_ssl_certificate() {
    ...
    case $CERT_CHOICE in
        1)
            ...
            local cert_log
            cert_log="/var/log/flux-core/certbot_${app_name}_$(date +%Y%m%d_%H%M%S).log"
