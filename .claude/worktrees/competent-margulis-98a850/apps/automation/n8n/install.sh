...
if ! { run_sudo docker exec postgres psql -U "$POSTGRES_USER" -tc "SELECT 1 FROM pg_database WHERE datname = '$N8N_DB_NAME'" | grep -q 1 || \
    run_sudo docker exec postgres psql -U "$POSTGRES_USER" -c "CREATE DATABASE $N8N_DB_NAME;"; }; then
    log_error "Failed to create database: $N8N_DB_NAME"
    exit 1
fi

# Create n8n user (authenticate with postgres superuser)
if ! { run_sudo docker exec postgres psql -U "$POSTGRES_USER" -tc "SELECT 1 FROM pg_roles WHERE rolname = '$N8N_DB_USER'" | grep -q 1 || \
    run_sudo docker exec postgres psql -U "$POSTGRES_USER" -c "CREATE USER $N8N_DB_USER WITH ENCRYPTED PASSWORD '$N8N_DB_PASSWORD';"; }; then
...
if ! { run_sudo docker exec postgres psql -U "$POSTGRES_USER" -c "GRANT ALL PRIVILEGES ON DATABASE $N8N_DB_NAME TO $N8N_DB_USER;" && \
run_sudo docker exec postgres psql -U "$POSTGRES_USER" -d "$N8N_DB_NAME" -c "GRANT ALL ON SCHEMA public TO $N8N_DB_USER;"; }; then
