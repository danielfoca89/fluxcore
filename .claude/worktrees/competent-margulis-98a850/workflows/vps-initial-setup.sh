#!/usr/bin/env bash
# shellcheck disable=SC2155

# ==============================================================================
# VPS INITIAL SETUP & HARDENING
# Universal VPS configuration for Ubuntu, Debian, AlmaLinux, etc.
# Run this LOCALLY on the VPS (not via SSH)
# ==============================================================================

set -euo pipefail

# Determine script directory and load all libraries via init.sh
SCRIPT_DIR="$(dirname "$(dirname "$(readlink -f "${BASH_SOURCE[0]}")")")"
export SCRIPT_DIR
if [[ -f "${SCRIPT_DIR}/lib/init.sh" ]]; then
    source "${SCRIPT_DIR}/lib/init.sh"
else
    # Fallback: load libraries individually
    source "${SCRIPT_DIR}/lib/utils.sh"
    source "${SCRIPT_DIR}/lib/os-detect.sh"
    source "${SCRIPT_DIR}/lib/secrets.sh"
fi

# Banner
print_banner() {
    clear
    cat << 'EOF'
╔═══════════════════════════════════════════════════════════╗
║           VPS ORCHESTRATOR                                ║
║           Initial Setup & Hardening                       ║
╚═══════════════════════════════════════════════════════════╝
EOF
    echo ""
    log_info "Detected OS: $(get_os_info)"
    echo ""
    
    # Initialize audit logging
    audit_log "VPS_SETUP_START" "system" "$(get_os_info)"
}

# Gather user input
gather_information() {
    log_step "Configuration Input Required"
    echo ""
    
    # New username
    while true; do
        echo -ne "${YELLOW}Enter new admin username:${NC} "
        read -r NEW_USER
        
        if [ -z "$NEW_USER" ]; then
            log_error "Username cannot be empty"
            continue
        fi
        
        if [[ ! "$NEW_USER" =~ ^[a-z_][a-z0-9_-]{2,31}$ ]]; then
            log_error "Invalid username (use lowercase, 3-32 chars)"
            continue
        fi
        
        if id "$NEW_USER" &>/dev/null; then
            log_warn "User $NEW_USER already exists"
            echo -ne "${YELLOW}Use existing user? (y/n):${NC} "
            read -r choice
            if [[ "$choice" =~ ^[Yy]$ ]]; then
                break
            fi
            continue
        fi
        
        break
    done
    
    # New admin user password - generated by lib/secrets.sh
    # Stored in GENERATED_USER_PASSWORD for display at end of setup (never re-shown)
    GENERATED_USER_PASSWORD=$(generate_system_password 20)
    log_info "Strong password auto-generated for user '$NEW_USER'."
    
    # SSH port
    while true; do
        echo -ne "${YELLOW}Enter new SSH port [Default port 22]:${NC} "
        read -r SSH_PORT
        SSH_PORT=${SSH_PORT:-22}
        
        if [[ ! "$SSH_PORT" =~ ^[0-9]+$ ]] || [ "$SSH_PORT" -lt 1 ] || [ "$SSH_PORT" -gt 65535 ]; then
            log_error "Port must be between 1 and 65535"
            continue
        fi
        
        break
    done
    
    echo ""
    log_info "Configuration Summary:"
    echo "  Username: $NEW_USER"
    echo "  SSH Port: $SSH_PORT"
    echo ""
    
    echo "Your connection will be interrupted when SSH is restarted."
    echo "After setup, connect using the new username and port."
    echo ""

    echo -ne "${YELLOW}Proceed with setup? (yes/no):${NC} "
    read -r confirm
    if [[ "$confirm" != "yes" ]]; then
        log_info "Setup cancelled"
        exit 0
    fi
}

# System update
system_update() {
    log_step "Step 1: System Update"
    
    if is_debian_based; then
        # Wait for apt locks
        while fuser /var/lib/dpkg/lock >/dev/null 2>&1 || fuser /var/lib/apt/lists/lock >/dev/null 2>&1; do
            log_info "Waiting for other package managers to finish..."
            sleep 5
        done
    fi
    
    log_info "Updating package index..."
    pkg_update
    
    log_info "Upgrading packages..."
    if is_debian_based; then
        run_sudo env DEBIAN_FRONTEND=noninteractive apt-get upgrade -y
        run_sudo apt-get autoremove -y
    elif is_rhel_based; then
        run_sudo dnf upgrade -y || run_sudo yum upgrade -y
        run_sudo dnf autoremove -y || run_sudo yum autoremove -y
    fi
    
    log_success "System updated"
}

# Install security tools
install_security_tools() {
    log_step "Step 2: Installing Security Tools"
    
    local packages=""
    
    # Common packages for all distros
    packages="curl wget git htop vim"
    
    # Add distro-specific packages
    if is_debian_based; then
        packages="$packages sudo ufw fail2ban auditd chrony unattended-upgrades apt-listchanges net-tools libpam-tmpdir"
    elif is_rhel_based; then
        packages="$packages sudo firewalld fail2ban audit chrony dnf-automatic net-tools"
    fi
    
    log_info "Installing: $packages"
    pkg_install $packages
    
    log_success "Security tools installed"
}

# Create admin user
create_admin_user() {
    log_step "Step 3: Setting Up Admin User"
    
    # Determine sudo group based on OS
    local SUDO_GROUP
    if is_debian_based; then
        SUDO_GROUP="sudo"
    elif is_rhel_based; then
        SUDO_GROUP="wheel"
    else
        SUDO_GROUP="sudo"
    fi
    
    if ! id "$NEW_USER" &>/dev/null; then
        run_sudo useradd -m -s /bin/bash "$NEW_USER"
        echo "${NEW_USER}:${GENERATED_USER_PASSWORD}" | run_sudo chpasswd
        run_sudo usermod -aG "$SUDO_GROUP" "$NEW_USER"
        run_sudo loginctl enable-linger "$NEW_USER"
        log_success "User $NEW_USER created, added to $SUDO_GROUP group, and linger enabled"
    else
        echo "${NEW_USER}:${GENERATED_USER_PASSWORD}" | run_sudo chpasswd
        run_sudo usermod -aG "$SUDO_GROUP" "$NEW_USER"
        run_sudo loginctl enable-linger "$NEW_USER"
        log_success "User $NEW_USER updated, added to $SUDO_GROUP group, and linger enabled"
    fi

    # Set root password (required for `su -` from user account)
    # Root SSH login remains DISABLED (key-only via authorized_keys + PermitRootLogin no)
    GENERATED_ROOT_PASSWORD=$(generate_system_password 20)
    echo "root:${GENERATED_ROOT_PASSWORD}" | run_sudo chpasswd
    log_success "Root password set (use 'su -' to switch to root, SSH login for root is disabled)"

    # Setup user home variable (used by both the fix and SSH setup below)
    USER_HOME="/home/$NEW_USER"

    # --- SYSTEMCTL --USER FIX ---
    # loginctl enable-linger (above) persists the runtime dir across reboots,
    # but the dir itself doesn't exist until a full login - we force-create it now.
    local USER_UID
    USER_UID=$(id -u "$NEW_USER")
    run_sudo mkdir -p "/run/user/$USER_UID"
    run_sudo chown "$NEW_USER:$NEW_USER" "/run/user/$USER_UID"
    run_sudo chmod 700 "/run/user/$USER_UID"
    log_success "XDG_RUNTIME_DIR /run/user/$USER_UID created"

    # Inject env vars into .bashrc so every terminal session is ready to use
    # systemctl --user, Docker rootless, PM2, etc. without manual setup.
    {
        echo ""
        echo "# Systemd User Services Fix"
        echo "export XDG_RUNTIME_DIR=/run/user/\$(id -u)"
        echo "export DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/\$(id -u)/bus"
        echo ""
        echo "# Standard paths for utilities and scripts (prioritizing /usr/local/bin for overrides)"
        echo "export PATH=\"\$HOME/.local/bin:\$HOME/bin:/usr/local/bin:\$PATH\""
        echo ""
        echo "# Standard umask for development (files=644, dirs=755)"
        echo "umask 022"
    } | run_sudo tee -a "$USER_HOME/.bashrc" > /dev/null

    log_success "Systemd user environment + umask 022 injected into $USER_HOME/.bashrc"
    # --- END SYSTEMCTL FIX ---

    # Setup SSH directory
    run_sudo mkdir -p "$USER_HOME/.ssh"
    run_sudo chmod 700 "$USER_HOME/.ssh"
    
    # Copy root keys if they exist
    if [ -f "/root/.ssh/authorized_keys" ]; then
        log_info "Copying SSH keys from root..."
        run_sudo cp /root/.ssh/authorized_keys "$USER_HOME/.ssh/authorized_keys"
    else
        log_warn "No SSH keys found in /root/.ssh/authorized_keys"
        echo -ne "${YELLOW}Paste your SSH public key now (or press Enter to skip):${NC} "
        read -r SSH_KEY_INPUT
        if [ -n "$SSH_KEY_INPUT" ]; then
            # Validate that the input looks like a valid SSH public key
            if echo "$SSH_KEY_INPUT" | ssh-keygen -l -f - &>/dev/null 2>&1; then
                echo "$SSH_KEY_INPUT" | run_sudo tee "$USER_HOME/.ssh/authorized_keys" > /dev/null
                log_success "SSH public key validated and saved"
            else
                log_warn "Invalid SSH public key format! Skipping - add your key manually later."
                run_sudo touch "$USER_HOME/.ssh/authorized_keys"
            fi
        else
            log_warn "No SSH key provided! You may be locked out if password auth is disabled."
            run_sudo touch "$USER_HOME/.ssh/authorized_keys"
        fi
    fi
    
    run_sudo chmod 600 "$USER_HOME/.ssh/authorized_keys"
    run_sudo chown -R "$NEW_USER:$NEW_USER" "$USER_HOME/.ssh"
    
    # Move repository if it's not already in the new user's home
    REPO_MOVED=false
    REPO_BASENAME=$(basename "$SCRIPT_DIR")
    if [[ "$SCRIPT_DIR" != "$USER_HOME/$REPO_BASENAME" ]]; then
        log_info "Moving repository to user home from $SCRIPT_DIR..."
        run_sudo mv "$SCRIPT_DIR" "$USER_HOME/$REPO_BASENAME"
        run_sudo chown -R "$NEW_USER:$NEW_USER" "$USER_HOME/$REPO_BASENAME"
        REPO_MOVED=true
        log_success "Repository moved to $USER_HOME/$REPO_BASENAME"
    fi
    
    # Fix ownership on the Home directory
    run_sudo chown -R "$NEW_USER:$NEW_USER" "$USER_HOME"

    # Group permissions for globally installed libraries and binaries
    # Apply SetGID only to /usr/local (not all of /opt, to avoid breaking third-party apps)
    if getent group "$SUDO_GROUP" &>/dev/null; then
        run_sudo mkdir -p /usr/local/bin /usr/local/lib /usr/local/sbin
        run_sudo chown -R root:"$SUDO_GROUP" /usr/local/bin /usr/local/lib /usr/local/sbin
        run_sudo chmod -R g+rwxs /usr/local/bin /usr/local/lib /usr/local/sbin
        # For /opt, create a dedicated admin directory instead of modifying all of /opt
        run_sudo mkdir -p /opt/admin
        run_sudo chown root:"$SUDO_GROUP" /opt/admin
        run_sudo chmod g+rwxs /opt/admin
        log_success "Group permissions (setgid) applied to /usr/local/* and /opt/admin for group $SUDO_GROUP"
    else
        log_warn "Group $SUDO_GROUP does not exist. Skipping global directory permissions."
    fi

    log_success "User configuration completed"
}

# Kernel and system hardening
kernel_hardening() {
    log_step "Step 4: Kernel Hardening"
    
    # Secure shared memory
    if ! grep -q "/dev/shm" /etc/fstab; then
        echo "tmpfs /dev/shm tmpfs defaults,noexec,nosuid 0 0" | run_sudo tee -a /etc/fstab > /dev/null
        run_sudo mount -o remount /dev/shm 2>/dev/null || true
    fi
    
    # Sysctl hardening
    run_sudo bash -c "cat > /etc/sysctl.d/99-security.conf" <<'EOF'
# IP Forwarding
net.ipv4.ip_forward = 0

# SYN flood protection + DoS tuning
net.ipv4.tcp_syncookies = 1
net.ipv4.tcp_syn_retries = 2
net.ipv4.tcp_synack_retries = 2
net.ipv4.tcp_max_syn_backlog = 65535
net.core.somaxconn = 65535

# IP spoofing protection
net.ipv4.conf.all.rp_filter = 1
net.ipv4.conf.default.rp_filter = 1

# Disable IP source routing
net.ipv4.conf.all.accept_source_route = 0
net.ipv4.conf.default.accept_source_route = 0

# Ignore ICMP redirects (IPv4 + IPv6)
net.ipv4.conf.all.accept_redirects = 0
net.ipv4.conf.default.accept_redirects = 0
net.ipv4.conf.all.secure_redirects = 0
net.ipv4.conf.default.secure_redirects = 0
net.ipv6.conf.all.accept_redirects = 0
net.ipv6.conf.default.accept_redirects = 0

# Ignore ICMP broadcast requests
net.ipv4.icmp_echo_ignore_broadcasts = 1
net.ipv4.icmp_ignore_bogus_error_responses = 1

# Log suspicious packets
net.ipv4.conf.all.log_martians = 1
net.ipv4.conf.default.log_martians = 1

# Kernel security
kernel.kptr_restrict = 2
kernel.dmesg_restrict = 1
kernel.unprivileged_bpf_disabled = 1
net.core.bpf_jit_harden = 2

# ASLR - Full address space randomization (anti exploit)
kernel.randomize_va_space = 2

# Disable core dumps (prevent credential leaks in dump files)
fs.suid_dumpable = 0
EOF

    run_sudo sysctl -p /etc/sysctl.d/99-security.conf >/dev/null
    
    log_success "Kernel hardened"
}

# Configure SSH
configure_ssh() {
    log_step "Step 5: SSH Hardening"
    
    # Backup original config before overwriting
    run_sudo cp /etc/ssh/sshd_config /etc/ssh/sshd_config.backup_$(date +%Y%m%d_%H%M%S)

    # Build hardened sshd_config. Notes on directive choices:
    #   KbdInteractiveAuthentication: replaces deprecated ChallengeResponseAuthentication (OpenSSH 8.7+)
    #   AllowTcpForwarding/AgentForwarding: disabled to reduce lateral-movement risk
    #   MaxSessions 2: limits concurrent multiplexed channels per connection
    SSHD_CONFIG="# SSH Daemon Configuration - Hardened
# Generated by vps-initial-setup.sh on $(date)

# Port and Protocol
Port $SSH_PORT
AddressFamily inet

# Authentication
PermitRootLogin no
PubkeyAuthentication yes
PasswordAuthentication no
PermitEmptyPasswords no
KbdInteractiveAuthentication no
UsePAM yes

# Security
X11Forwarding no
PrintMotd no
PrintLastLog no
PermitUserEnvironment no
AllowTcpForwarding no
AllowAgentForwarding no
GatewayPorts no
MaxAuthTries 10
MaxSessions 2

# Timeouts
ClientAliveInterval 300
ClientAliveCountMax 2
LoginGraceTime 60

# Logging
SyslogFacility AUTH
LogLevel INFO

# Allowed users
AllowUsers $NEW_USER

# Include additional configs if they exist
Include /etc/ssh/sshd_config.d/*.conf"
    
    # Write SSH config with sudo
    echo "$SSHD_CONFIG" | run_sudo tee /etc/ssh/sshd_config > /dev/null
    
    # Validate config before anything restarts — catches unknown directives early
    if ! run_sudo sshd -t 2>/dev/null; then
        log_error "SSH configuration is invalid!"
        log_error "Restoring backup..."
        LATEST_BACKUP=$(ls -t /etc/ssh/sshd_config.backup_* 2>/dev/null | head -1)
        if [ -n "$LATEST_BACKUP" ]; then
            run_sudo cp "$LATEST_BACKUP" /etc/ssh/sshd_config
        fi
        exit 1
    fi
    
    log_success "SSH configuration written and validated (port $SSH_PORT)"
    log_info "SSH will be restarted at the end of setup"
}

# Configure firewall
configure_firewall() {
    log_step "Step 6: Firewall Configuration"
    
    # Get firewall type for this OS
    FIREWALL_TYPE=$(get_firewall_service)
    log_info "Using firewall: $FIREWALL_TYPE"
    
    if [ "$FIREWALL_TYPE" = "ufw" ]; then
        # UFW (Ubuntu/Debian)
        run_sudo ufw --force reset
        
        # Default policies
        run_sudo ufw default deny incoming
        run_sudo ufw default allow outgoing
        
        # IMPORTANT: Allow SSH on NEW PORT before enabling firewall
        log_info "Allowing SSH on port $SSH_PORT..."
        run_sudo ufw allow "$SSH_PORT/tcp" comment "SSH"
        
        # Enable UFW
        log_info "Enabling firewall..."
        run_sudo ufw --force enable
        
        log_success "Firewall enabled with SSH port $SSH_PORT allowed"
        log_info "HTTP/HTTPS ports will be opened when nginx is installed"
        
    elif [ "$FIREWALL_TYPE" = "firewalld" ]; then
        # firewalld (RHEL/CentOS/AlmaLinux)
        run_sudo systemctl enable --now firewalld
        
        # Allow SSH port
        run_sudo firewall-cmd --permanent --add-port="${SSH_PORT}/tcp"
        
        # Reload firewall
        run_sudo firewall-cmd --reload
        
        log_success "Firewall enabled with SSH port $SSH_PORT allowed"
        log_info "HTTP/HTTPS ports will be opened when nginx is installed"
    else
        log_error "No supported firewall found!"
        return 1
    fi
    
    log_success "Firewall configured and enabled"
}

# Configure Fail2ban
configure_fail2ban() {
    log_step "Step 7: Fail2ban Configuration"
    
    # Get correct log path for this OS
    local SSH_LOG_PATH
    if is_debian_based; then
        SSH_LOG_PATH="/var/log/auth.log"
    elif is_rhel_based; then
        SSH_LOG_PATH="/var/log/secure"
    else
        SSH_LOG_PATH="/var/log/auth.log"
    fi
    
    # Generate fail2ban config with expanded variables
    FAIL2BAN_CONFIG="[DEFAULT]
bantime = 86400
findtime = 600
maxretry = 3
destemail = root@localhost
sendername = Fail2Ban
action = %(action_mw)s

# SQLite persistent DB - bans survive fail2ban restarts
dbfile = /var/lib/fail2ban/fail2ban.sqlite3
dbpurgeage = 86400

[sshd]
enabled = true
port = $SSH_PORT
logpath = $SSH_LOG_PATH
maxretry = 3
bantime = 86400

# Recidivists: 2nd offence within 24h = 7 day ban
[sshd-recidivists]
enabled = true
filter = sshd
port = $SSH_PORT
logpath = $SSH_LOG_PATH
maxretry = 1
findtime = 86400
bantime = 604800"
    
    # Write fail2ban config with sudo
    echo "$FAIL2BAN_CONFIG" | run_sudo tee /etc/fail2ban/jail.local > /dev/null
    
    service_restart fail2ban
    service_enable fail2ban
    
    log_success "Fail2ban configured (SQLite persistent, 24h SSH ban, recidivists jail)"
}

# Configure audit logs
configure_audit() {
    log_step "Step 8: Audit Logging"
    
    run_sudo bash -c "cat > /etc/audit/rules.d/audit.rules" <<'EOF'
# Monitor identity changes
-w /etc/passwd -p wa -k identity
-w /etc/group -p wa -k identity
-w /etc/shadow -p wa -k identity

# Monitor SSH configuration
-w /etc/ssh/sshd_config -p warx -k sshd_config

# Monitor sudo usage + sudoers changes
-w /etc/sudoers -p wa -k sudoers
-w /etc/sudoers.d/ -p wa -k sudoers
-w /var/log/sudo.log -p wa -k sudo_log

# Monitor privilege escalation binaries (sudo, su)
-w /usr/bin/sudo -p x -k privilege_escalation
-w /bin/su -p x -k privilege_escalation

# Monitor cron (common persistence attack vector)
-w /etc/cron.d/ -p wa -k cron
-w /etc/crontab -p wa -k cron
-w /var/spool/cron/ -p wa -k cron

# Monitor kernel module loading (rootkit detection)
-w /sbin/insmod -p x -k kernel_modules
-w /sbin/rmmod -p x -k kernel_modules
-w /sbin/modprobe -p x -k kernel_modules

# Monitor network configuration
-w /etc/network/ -p wa -k network

# Monitor local binary directories (high-value attack surface)
-w /usr/local/bin -p wa -k local_binaries
-w /usr/local/sbin -p wa -k local_binaries
-w /opt/admin -p wa -k local_binaries

# Monitor system calls (execve)
-a always,exit -F arch=b64 -S execve -k exec
EOF

    run_sudo systemctl restart auditd
    
    log_success "Audit logging configured (identity, SSH, sudo escalation, cron, kernel modules)"
}

# Configure automatic updates
configure_auto_updates() {
    log_step "Step 9: Automatic Security Updates"

    if ! is_debian_based; then
        log_info "Skipping unattended-upgrades setup (not Debian/Ubuntu based)"
        return 0
    fi

    run_sudo bash -c "cat > /etc/apt/apt.conf.d/50unattended-upgrades" <<'EOF'
Unattended-Upgrade::Allowed-Origins {
    "${distro_id}:${distro_codename}-security";
    "${distro_id}ESMApps:${distro_codename}-apps-security";
    "${distro_id}ESM:${distro_codename}-infra-security";
};
Unattended-Upgrade::AutoFixInterruptedDpkg "true";
Unattended-Upgrade::MinimalSteps "true";
Unattended-Upgrade::Remove-Unused-Dependencies "true";
Unattended-Upgrade::Automatic-Reboot "false";
EOF
    
    run_sudo bash -c "cat > /etc/apt/apt.conf.d/20auto-upgrades" <<'EOF'
APT::Periodic::Update-Package-Lists "1";
APT::Periodic::Download-Upgradeable-Packages "1";
APT::Periodic::AutocleanInterval "7";
APT::Periodic::Unattended-Upgrade "1";
EOF
    
    log_success "Automatic updates configured"
}

# Set custom MOTD
set_motd() {
    log_step "Step 10: Setting MOTD"
    
    run_sudo rm -f /etc/update-motd.d/* 2>/dev/null || true
    
    run_sudo bash -c "cat > /etc/motd" <<'EOF'
═══════════════════════════════════════════════════════════
 ⚠️  WARNING: RESTRICTED ACCESS SYSTEM
═══════════════════════════════════════════════════════════

 All activities on this system are logged and monitored.
 Unauthorized access will be reported and prosecuted.
 
 This server is configured with enterprise security:
 • SSH Hardening
 • Firewall Protection (UFW)
 • Intrusion Prevention (Fail2ban)
 • Audit Logging
 • Automatic Security Updates
 
═══════════════════════════════════════════════════════════
EOF
    
    log_success "MOTD configured"
}

# Allow admin user to reboot/shutdown/suspend without sudo
# and add to docker group if Docker is already installed
configure_user_flexibility() {
    log_step "Step 12.2: Configuring User Flexibility (polkit + groups)"

    # --- Polkit rule: reboot / poweroff / suspend without sudo ---
    # Works on all modern systemd systems (Debian 11+, Ubuntu 22.04+)
    local polkit_rules_dir="/etc/polkit-1/rules.d"
    if [[ -d "$polkit_rules_dir" ]]; then
        run_sudo bash -c "cat > ${polkit_rules_dir}/49-admin-power.rules" <<'PKEOF'
// Allow members of the sudo/wheel group to reboot, power off, and suspend
// without requiring a password or sudo prefix.
polkit.addRule(function(action, subject) {
    var powerActions = [
        "org.freedesktop.login1.reboot",
        "org.freedesktop.login1.reboot-multiple-sessions",
        "org.freedesktop.login1.power-off",
        "org.freedesktop.login1.power-off-multiple-sessions",
        "org.freedesktop.login1.suspend",
        "org.freedesktop.login1.hibernate"
    ];
    if (powerActions.indexOf(action.id) !== -1 &&
        (subject.isInGroup("sudo") || subject.isInGroup("wheel"))) {
        return polkit.Result.YES;
    }
});
PKEOF
        run_sudo chmod 644 "${polkit_rules_dir}/49-admin-power.rules"
        log_success "Polkit rule applied: reboot/poweroff/suspend work without sudo prefix"
    else
        # Fallback: older pkla format (Debian 10 / Ubuntu 20.04)
        local pkla_dir="/etc/polkit-1/localauthority/50-local.d"
        run_sudo mkdir -p "$pkla_dir"
        run_sudo bash -c "cat > ${pkla_dir}/49-admin-power.pkla" <<'PKEOF'
[Allow admin group to manage power state]
Identity=unix-group:sudo;unix-group:wheel
Action=org.freedesktop.login1.reboot;org.freedesktop.login1.reboot-multiple-sessions;org.freedesktop.login1.power-off;org.freedesktop.login1.power-off-multiple-sessions;org.freedesktop.login1.suspend
ResultAny=yes
ResultInactive=yes
ResultActive=yes
PKEOF
        run_sudo chmod 644 "${pkla_dir}/49-admin-power.pkla"
        log_success "Polkit pkla rule applied (legacy): reboot/poweroff work without sudo prefix"
    fi

    # --- Docker group membership (if docker is already installed) ---
    if getent group docker &>/dev/null; then
        run_sudo usermod -aG docker "$NEW_USER"
        log_success "User '$NEW_USER' added to docker group (docker commands work without sudo after re-login)"
    else
        log_info "Docker not installed yet — user will be added to docker group automatically by the Docker installer"
    fi

    # --- adm group: read system logs without sudo ---
    # Gives access to /var/log/syslog, /var/log/auth.log, /var/log/kern.log etc.
    if getent group adm &>/dev/null; then
        run_sudo usermod -aG adm "$NEW_USER"
        log_success "User '$NEW_USER' added to adm group (read /var/log/* without sudo)"
    fi

    # --- systemd-journal group: full journalctl without sudo ---
    if getent group systemd-journal &>/dev/null; then
        run_sudo usermod -aG systemd-journal "$NEW_USER"
        log_success "User '$NEW_USER' added to systemd-journal group (journalctl -u <service> without sudo)"
    fi

    # --- Polkit rule: systemctl manage services without sudo prefix ---
    if [[ -d "$polkit_rules_dir" ]]; then
        run_sudo bash -c "cat > ${polkit_rules_dir}/49-admin-systemctl.rules" <<'PKEOF'
// Allow members of sudo/wheel group to start, stop, restart and reload
// systemd services without requiring the sudo prefix.
polkit.addRule(function(action, subject) {
    if (action.id === "org.freedesktop.systemd1.manage-units" &&
        (subject.isInGroup("sudo") || subject.isInGroup("wheel"))) {
        return polkit.Result.YES;
    }
});
PKEOF
        run_sudo chmod 644 "${polkit_rules_dir}/49-admin-systemctl.rules"
        log_success "Polkit rule applied: systemctl start/stop/restart works without sudo prefix"
    fi

    log_success "User flexibility configured"
}

# Final checks
final_checks() {
    log_step "Step 11: Final Verification"
    
    # SSH - detect correct service name per distro
    local SSH_SVC
    SSH_SVC=$(get_ssh_service_name)
    if systemctl is-active --quiet "$SSH_SVC" 2>/dev/null; then
        log_success "SSH ($SSH_SVC) is running on port $SSH_PORT"
    else
        log_warn "SSH ($SSH_SVC) is NOT running!"
    fi

    # Core security services
    local services=("fail2ban" "auditd")
    for service in "${services[@]}"; do
        if systemctl is-active --quiet "$service" 2>/dev/null; then
            log_success "$service is running"
        else
            log_warn "$service is NOT running"
        fi
    done
    
    # Firewall status - supports both UFW (Debian) and firewalld (RHEL)
    if systemctl is-active --quiet ufw 2>/dev/null; then
        log_info "Firewall (UFW) status:"
        run_sudo ufw status numbered
    elif systemctl is-active --quiet firewalld 2>/dev/null; then
        log_info "Firewall (firewalld) status:"
        run_sudo firewall-cmd --list-all
    else
        log_warn "No active firewall detected!"
    fi
}

# Main execution
main() {
    # Check if root
    if [ "$(id -u)" -ne 0 ]; then
        log_error "This script must be run as root"
        exit 1
    fi
    
    # Check if running over SSH - DANGEROUS!
    if [ -n "${SSH_CONNECTION:-}" ] || [ -n "${SSH_CLIENT:-}" ] || [ -n "${SSH_TTY:-}" ]; then
        echo ""
        log_error "═══════════════════════════════════════════════════════════"
        log_error "  DANGER: This script is being run over SSH!"
        log_error "═══════════════════════════════════════════════════════════"
        echo ""
        log_warn "This script will RESTART SSH service and change the port."
        log_warn "Running it over SSH will disconnect you and may lock you out!"
        echo ""
        log_info "How to run safely:"
        echo "  1. Use VPS provider's web console (VNC/noVNC)"
        echo "  2. Use IPMI/iKVM/BMC console"
        echo "  3. Have physical access to the server"
        echo ""
        echo -ne "${YELLOW}Are you ABSOLUTELY SURE you want to continue? (type 'FORCE' to proceed):${NC} "
        read -r confirm
        if [[ "$confirm" != "FORCE" ]]; then
            log_info "Setup cancelled - use web console instead"
            exit 0
        fi
        echo ""
        log_warn "⚠️  YOU HAVE BEEN WARNED! Proceeding anyway..."
        echo ""
        sleep 3
    fi
    
    print_banner
    
    log_warn "This script will configure and harden your VPS"
    log_warn "Make sure you have console access in case of issues!"
    echo ""
    
    gather_information
    
    echo ""
    log_info "Starting VPS setup and hardening..."
    echo ""
    
    system_update
    install_security_tools
    create_admin_user
    kernel_hardening
    configure_ssh
    configure_firewall
    configure_fail2ban
    configure_audit
    configure_auto_updates
    set_motd
    configure_sudo_security   # Step 12: sudo hardening (Debian/Ubuntu)
    configure_user_flexibility # Step 12.2: polkit + group membership for passwordless UX
    
    # Securizează grupul administrativ (doar root poate modifica grupul sudo/wheel)
    log_step "Step 12.1: Restricting Password & User Management"
    local ADMIN_GROUP
    if is_debian_based; then ADMIN_GROUP="sudo"; else ADMIN_GROUP="wheel"; fi

    run_sudo bash -c "cat > /etc/sudoers.d/01-restrict-sensitive-cmds" <<EOF
# Admin users can do everything EXCEPT change passwords or create new accounts.
# Full command logging is active via 99-hardening.
Cmnd_Alias FORBIDDEN_CMDS = /usr/bin/passwd, /usr/sbin/chpasswd, /usr/bin/chpasswd, /usr/sbin/useradd, /usr/sbin/adduser
%${ADMIN_GROUP} ALL=(ALL:ALL) !FORBIDDEN_CMDS
EOF
    run_sudo chmod 0440 /etc/sudoers.d/01-restrict-sensitive-cmds
    log_success "Restrictions applied: password changes and user creation are forbidden for ${ADMIN_GROUP} group"

    final_checks
    
    echo ""
    echo ""
    log_success "═══════════════════════════════════════════"
    log_success "  VPS Configuration Completed!"
    log_success "═══════════════════════════════════════════"
    echo ""
    log_info "Configuration Summary:"
    echo "  • New User:     $NEW_USER"
    echo "  • SSH Port:     $SSH_PORT (NOT YET ACTIVE)"
    echo "  • Server IP:    $(hostname -I | awk '{print $1}')"
    echo ""
    log_warn "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    log_warn "  ⚠️  SAVE THESE CREDENTIALS - SHOWN ONLY ONCE"
    log_warn "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo ""
    echo -e "${GREEN}  Admin User: ${NC}${NEW_USER}"
    echo -e "${GREEN}  Password:   ${NC}${GENERATED_USER_PASSWORD}"
    echo ""
    echo -e "${YELLOW}  Root User:  ${NC}root"
    echo -e "${YELLOW}  Password:   ${NC}${GENERATED_ROOT_PASSWORD}"
    echo -e "${YELLOW}  Access:     ${NC}Run 'su -' from $NEW_USER (SSH root login is DISABLED)"
    unset GENERATED_USER_PASSWORD GENERATED_ROOT_PASSWORD  # clear from memory after display
    echo ""
    if [ "$REPO_MOVED" = true ]; then
        log_info "Repository moved to: /home/$NEW_USER/$REPO_BASENAME"
    fi
    
    echo ""
    log_warn "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    log_warn "  FINAL STEP: SSH SERVICE RESTART REQUIRED"
    log_warn "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo ""
    log_error "⚠️  WARNING: SSH will now restart on port $SSH_PORT"
    echo ""
    log_info "What will happen:"
    echo "  1. SSH service restarts with new configuration"
    echo "  2. SSH will listen ONLY on port $SSH_PORT"
    echo "  3. Root login will be DISABLED"
    echo "  4. You can only login as: $NEW_USER"
    echo ""
    
    if [ -n "${SSH_CONNECTION:-}" ]; then
        log_error "YOU ARE CONNECTED VIA SSH - YOU WILL BE DISCONNECTED!"
    fi
    
    echo ""
    log_info "After SSH restarts, connect with:"
    echo ""
    echo "  ssh $NEW_USER@$(hostname -I | awk '{print $1}') -p $SSH_PORT"
    echo ""
    echo "  Password: (the one you set for $NEW_USER)"
    echo ""
    log_warn "If you cannot connect, use VPS console to troubleshoot"
    echo ""
    
    if [[ "${FORCE_YES:-}" == "1" ]]; then
        final_confirm="YES"
        echo -e "${YELLOW}Restarting SSH automatically (FORCE_YES is set)...${NC}"
    else
        echo -ne "${YELLOW}Restart SSH NOW and apply changes? (type 'YES' to confirm):${NC} "
        read -r final_confirm
    fi
    
    if [[ "$final_confirm" != "YES" ]]; then
        echo ""
        log_info "SSH restart cancelled"
        log_warn "Configuration is ready but NOT active yet"
        log_info "To activate manually, run: systemctl restart $(get_ssh_service_name)"
        exit 0
    fi
    
    echo ""
    log_info "Restarting SSH service..."
    
    # Get SSH service name
    SSH_SERVICE=$(get_ssh_service_name)
    
    # Restart SSH
    if systemctl restart "$SSH_SERVICE"; then
        log_success "SSH service restarted successfully!"
        echo ""
        log_success "✓ Setup Complete! SSH is now on port $SSH_PORT"
        audit_log "VPS_SETUP_COMPLETE" "system" "User: $NEW_USER, SSH Port: $SSH_PORT"
        echo ""
        log_warn "This session will now close."
        log_info "Reconnect with: ssh $NEW_USER@$(hostname -I | awk '{print $1}') -p $SSH_PORT"
        echo ""
        sleep 2
        exit 0
    else
        log_error "Failed to restart SSH!"
        audit_log "VPS_SETUP_FAILED" "system" "SSH restart failed" "FAILED"
        log_error "Configuration is still in place, but SSH may not be running"
        log_warn "Use VPS console to check: systemctl status $SSH_SERVICE"
        exit 1
    fi
}

# Run
main
