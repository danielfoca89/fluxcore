#!/usr/bin/env bash
# shellcheck disable=SC1091,SC2155,SC2086,SC2181,SC2010,SC2162
...
backup_postgres() {
    ...
    pg_user=$(get_secret "postgres" "POSTGRES_USER")
    pg_password=$(get_secret "postgres" "DB_PASSWORD")
    ...
    if run_sudo docker exec -e PGPASSWORD="$pg_password" postgres pg_dumpall -U "$pg_user" > "$backup_file"; then
        gzip -f "$backup_file"
...
backup_mariadb() {
    ...
    db_user=$(get_secret "mariadb" "DB_USER")
    db_password=$(get_secret "mariadb" "DB_PASSWORD")
    ...
    # Backup all databases
    if run_sudo docker exec mariadb sh -c "exec mariadb-dump --all-databases -uroot -p\"$db_password\"" > "$backup_file" 2>/dev/null; then
        run_sudo gzip -f "$backup_file"
        run_sudo chmod 600 "${backup_file}.gz"
...
backup_mongodb() {
    ...
    local db_user db_password
    db_user=$(get_secret "mongodb" "MONGO_INITDB_ROOT_USERNAME")
    db_password=$(get_secret "mongodb" "MONGO_INITDB_ROOT_PASSWORD")
    ...
    # Backup all databases
    if run_sudo docker exec mongodb mongodump --username="$db_user" --password="$db_password" --authenticationDatabase admin --out=/dump; then
        # Copy dump outside container and compress
        run_sudo docker cp mongodb:/dump "$backup_dir"
