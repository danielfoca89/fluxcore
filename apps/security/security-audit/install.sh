...
run_sudo tee /usr/local/bin/security-audit > /dev/null << 'EOFAUDIT'
#!/bin/bash
set -Eeuo pipefail
...

{
    ...
    echo "-----------------------------------"
    IMAGES=$(sudo docker images --format "{{.Repository}}:{{.Tag}}" | grep -v "<none>" | head -20)
    if [[ -n "$IMAGES" ]]; then
    ...
    echo "----------------------"
    sudo docker ps --format "table {{.Names}}\t{{.Image}}\t{{.Status}}\t{{.Ports}}" 2>&1 || echo "Error checking containers"
    echo ""
...
run_sudo tee /usr/local/bin/scan-docker-images > /dev/null << 'EOFSCAN'
#!/bin/bash
set -Eeuo pipefail
