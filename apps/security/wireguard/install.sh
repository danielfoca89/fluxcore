...
read -r -p "Enter server public IP/domain [$SERVER_IP]: " USER_SERVER_URL
SERVER_URL="${USER_SERVER_URL:-$SERVER_IP}"

read -r -p "WireGuard port [51820]: " USER_PORT
WG_PORT="${USER_PORT:-51820}"

read -r -p "Number of peers to create [3]: " USER_PEERS
PEERS="${USER_PEERS:-3}"
...
case "$1" in
    ...
    list)
        ...
        find "$WG_DIR/clients" -maxdepth 1 -name '*.conf' 2>/dev/null | xargs -r -n1 basename | sed 's/\.conf$//' || echo "No peers found"
        ;;
    show)
