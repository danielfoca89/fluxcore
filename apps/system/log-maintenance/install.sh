...
run_sudo tee /usr/local/bin/cleanup-logs > /dev/null << 'EOFCLEANUP'
#!/bin/bash
set -Eeuo pipefail
...
if command -v docker &> /dev/null; then
    ...
    if [[ "$STOPPED" -gt 0 ]]; then
        sudo docker ps -aq -f status=exited | xargs -r sudo docker rm >/dev/null 2>&1 || true
...
run_sudo tee /usr/local/bin/disk-report > /dev/null << 'EOFREPORT'
#!/bin/bash
