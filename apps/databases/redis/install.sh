...
if run_sudo grep -q "^# Commented by flux-core - bind" "$CONF_FILE"; then
    ...
    run_sudo sed -i "/^# Commented by flux-core - bind/{ n; s|^bind .*|bind $BIND_IPS|; }" "$CONF_FILE"
else
    ...
    if [[ -n "$BIND_LINE" ]]; then
        ...
        run_sudo sed -i "${BIND_LINE}s|^|# Commented by flux-core - |" "$CONF_FILE"
...
if run_sudo grep -q "^# Commented by flux-core - protected-mode" "$CONF_FILE"; then
    # Already modified - update the active line
    run_sudo sed -i "/^# Commented by flux-core - protected-mode/{ n; s|^protected-mode .*|protected-mode no|; }" "$CONF_FILE"
else
    ...
    if [[ -n "$PROT_LINE" ]]; then
        run_sudo sed -i "${PROT_LINE}s|^|# Commented by flux-core - |" "$CONF_FILE"
...
if run_sudo grep -q "^requirepass " "$CONF_FILE"; then
...
elif run_sudo grep -q "^# Commented by flux-core - # requirepass" "$CONF_FILE"; then
    # Already modified - update the active line
    run_sudo sed -i "/^# Commented by flux-core - # requirepass/{ n; s|^requirepass .*|requirepass $REDIS_PASSWORD|; }" "$CONF_FILE"
else
    ...
    if [[ -n "$REQ_LINE" ]]; then
        run_sudo sed -i "${REQ_LINE}s|^# |# Commented by flux-core - # |" "$CONF_FILE"
...
if run_sudo grep -q "^supervised " "$CONF_FILE"; then
    run_sudo sed -i '/^supervised /s/^/# Commented by flux-core - /' "$CONF_FILE"
    run_sudo sed -i '/# Commented by flux-core - supervised /a supervised systemd' "$CONF_FILE"
else
...
if run_sudo grep -q "^# Commented by flux-core - appendonly" "$CONF_FILE"; then
    # Already modified - update the active line
    run_sudo sed -i "/^# Commented by flux-core - appendonly/{ n; s|^appendonly .*|appendonly yes|; }" "$CONF_FILE"
else
    # First time - find active appendonly line, comment it, add ours
    APP_LINE=$(run_sudo grep -n "^appendonly " "$CONF_FILE" | head -1 | cut -d: -f1)
    if [[ -n "$APP_LINE" ]]; then
        run_sudo sed -i "${APP_LINE}s|^|# Commented by flux-core - |" "$CONF_FILE"
...
if run_sudo grep -q "^maxmemory-policy" "$CONF_FILE"; then
    run_sudo sed -i '/^maxmemory-policy /s/^/# Commented by flux-core - /' "$CONF_FILE"
    run_sudo sed -i '/# Commented by flux-core - maxmemory-policy /a maxmemory-policy allkeys-lru' "$CONF_FILE"
...
if run_sudo grep -q "^# Commented by flux-core - logfile" "$CONF_FILE"; then
    # Already modified - update the path
    run_sudo sed -i "/^# Commented by flux-core - logfile/{ n; s|^logfile .*|logfile $LOG_FILE|; }" "$CONF_FILE"
else
    # First time - find active logfile line, comment it, add ours
    LOG_LINE=$(run_sudo grep -n "^logfile " "$CONF_FILE" | head -1 | cut -d: -f1)
    if [[ -n "$LOG_LINE" ]]; then
        run_sudo sed -i "${LOG_LINE}s|^|# Commented by flux-core - |" "$CONF_FILE"
...
if run_sudo grep -q "^# Commented by flux-core - dir" "$CONF_FILE"; then
    # Already modified - update the path
    run_sudo sed -i "/^# Commented by flux-core - dir/{ n; s|^dir .*|dir $DATA_DIR|; }" "$CONF_FILE"
else
    # First time - find active dir line, comment it, add ours
    DIR_LINE=$(run_sudo grep -n "^dir " "$CONF_FILE" | head -1 | cut -d: -f1)
    if [[ -n "$DIR_LINE" ]]; then
        run_sudo sed -i "${DIR_LINE}s|^|# Commented by flux-core - |" "$CONF_FILE"
