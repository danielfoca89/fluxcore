...
# Backward compatibility for VERSION variable (uses FLUXCORE_VERSION from constants.sh)
VERSION="${FLUXCORE_VERSION:-dev}"
...
is_app_installed() {
    ...
    local app_dir
    app_dir=$(find "${SCRIPT_DIR}/apps" -name "$app_name" -type d -exec test -f "{}/install.sh" \; -print -quit)

    if [[ -n "$app_dir" && -f "${app_dir}/check.sh" ]]; then
        if bash "${app_dir}/check.sh"; then
            return 0
        else
            return 1
        fi
    ...
    # Fallback default: Docker container check
    command -v docker &>/dev/null && systemctl is-active --quiet docker 2>/dev/null || return 1
    
    # Check if we can run docker without interactive sudo to prevent password prompt on menu load
    local docker_cmd="docker"
    if ! docker info &>/dev/null; then
        if sudo -n docker info &>/dev/null; then
            docker_cmd="sudo docker"
        else
            return 1 # Cannot check without password prompt
        fi
    fi

    $docker_cmd ps -a --format '{{.Names}}' 2>/dev/null | grep -q "^${app_name}$" && return 0
    return 1
}
...
install_optional_dependencies() {
    ...
    for opt_dep in "${OPT_DEPS[@]}"; do
        ...
        if bash "${opt_script}/install.sh"; then
            log_success "Optional installed: $opt_dep"
        else
