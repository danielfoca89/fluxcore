...
# get_ssh_service_name — DEPRECATED: use version from os-detect.sh
...
# service_exists — DEPRECATED: use version from os-detect.sh
# Kept as fallback only if os-detect.sh is not loaded
...
# --- OS DETECTION ---
# NOTE: The authoritative detect_os() is in os-detect.sh.
# This is a backward-compatible wrapper that also sets OS_NAME (alias for OS_ID).
if ! declare -f detect_os >/dev/null 2>&1; then
    # Fallback: define a minimal detect_os if os-detect.sh is not loaded yet
    detect_os() {
...
EOF
    log_success "sudo configured: NOPASSWD for admin group + full command logging → /var/log/sudo.log"
}
