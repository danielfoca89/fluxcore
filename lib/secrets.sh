...
[[ -n "${_FLUXCORE_SECRETS_LOADED:-}" ]] && return 0
readonly _FLUXCORE_SECRETS_LOADED=1
...
save_secret() {
    ...
    # Remove existing entry if present, then append — avoids sed injection
    # (values can contain &, |, \ which break sed replacement syntax)
...
backup_all_secrets() {
    ...
    local cred_files
    cred_files=$(find "${SECRETS_DIR}" -maxdepth 1 -name ".env_*" 2>/dev/null)
    ...
    if tar -czf "$backup_file" -C "$SECRETS_DIR" $(find "$SECRETS_DIR" -maxdepth 1 -name ".env_*" -printf "%f\n") 2>/dev/null; then
...
restore_secrets_from_backup() {
    ...
    if tar -xzf "$backup_file" -C "$SECRETS_DIR"; then
    ...
    while IFS='=' read -r key value; do
        if [[ -n "$key" ]] && [[ ! "$key" =~ ^# ]]; then
            ...
            if [[ "$value_length" -gt 8 ]]; then
                ...
                masked="${clean_value:0:4}$(printf '%*s' "$mask_len" '' | tr ' ' '*')${clean_value: -4}"
            else
                masked="****"
            fi
...
list_all_secrets() {
    ...
    for env_file in "${SECRETS_DIR}"/.env_*; do
        if [[ -f "$env_file" ]]; then
            local app_name
            app_name=$(basename "$env_file" | sed 's/^\.env_//')
            local var_count
...
backup_secrets() {
    ...
    # shellcheck disable=SC2046
    if tar -czf "$backup_path" -C "$SECRETS_DIR" $(find "$SECRETS_DIR" -maxdepth 1 -name ".env_*" -printf "%f\n") 2>/dev/null; then
        chmod 600 "$backup_path"
        log_success "Backup created: $backup_path"
        
        # Keep only last 10 backups
        local backup_count
        backup_count=$(find "$BACKUP_DIR" -name "secrets_backup_*.tar.gz" | wc -l)
...
delete_secrets() {
    ...
    local backup_file
    backup_file="${env_file}.deleted_$(date +%Y%m%d_%H%M%S)"
